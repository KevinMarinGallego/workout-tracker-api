...
let workoutPlans = [
  {
    ...
    exercises: [
      {
        exerciseId: "d09f3bb4-35b6-4a7b-b5a1-09f01d3a28bc",
        name: "Sentadilla",
        sets: 4,
        reps: 10,
      },
      {
        exerciseId: "2a1f3bb4-33c6-1c7a-b5a1-99f01d3a45fg",
        name: "Press de banca",
        sets: 3,
        reps: 8,
      },
...
const getAllWorkoutPlans = (req, res) => {
  try {
    ...
    if (userId)
      result = result.filter((p) => p.userId === String(userId).trim());
    ...
    return res
      ...
      .json({
        success: false,
        error: "Error al listar workout plans",
...
const getWorkoutPlanById = (req, res) => {
  try {
    ...
    if (!plan) {
      return res
        .status(404)
        .json({ success: false, error: "Workout plan no encontrado" });
    }
  ...
  try {
    ...
    if (!userId || !title || !description || !Array.isArray(exercises)) {
      return res.status(400).json({
        success: false,
        error: 'userId, title, description y exercises son obligatorios'
      });
    }
    ...
    for (const it of exercises) {
      if (!isValidExerciseItem(it)) {
        return res.status(400).json({
          success: false,
          error: 'Cada ejercicio debe tener exerciseId, name, sets y reps'
        });
      }
    }
    ...
    return res.status(201).json({
      success: true,
      message: 'Workout plan creado',
      data: newPlan
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      error: 'Error al crear workout plan',
      message: error.message
    });
  }
};
...
const updateWorkoutPlan = (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: 'Workout plan actualizado',
      data: workoutPlans[idx]
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      error: 'Error al actualizar workout plan',
      message: error.message
    });
  }
};
  ...
  try {
    ...
    if (updates.exercises) {
      if (!Array.isArray(updates.exercises)) {
        return res.status(400).json({
          success: false,
          error: 'exercises debe ser un array'
        });
      }
    ...
    return res.status(200).json({
      success: true,
      message: 'Workout plan actualizado parcialmente',
      data: workoutPlans[idx]
    });
  } catch (error) {
    return res.status(500).json({
